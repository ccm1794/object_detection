...
class ObjectDetection(Node):
    def __init__(self, img_size: Union[Tuple[int, int], None] = (640, 640)
        ...
        self.video_capture_right = cv2.VideoCapture("/dev/v4l/by-id/usb-046d_罗技高清网络摄像机_C930c_E776BDCE-video-index0", cv2.CAP_V4L2)
        self.video_capture_right.set(cv2.CAP_PROP_FRAME_WIDTH, 640)
        self.video_capture_right.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)

        self.video_capture_left = cv2.VideoCapture("/dev/v4l/by-id/usb-046d_罗技高清网络摄像机_C930c_9EAC4AAE-video-index0", cv2.CAP_V4L2)
    ...
    def timer_callback(self):
        ...
        if ret_right and ret_left:
            # vconcat
            frame = cv2.vconcat([frame_left, frame_right])
