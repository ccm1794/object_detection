...
class ObjectDetection(Node):
    ...
    def timer_callback(self):
        ret_right, self.frame_right = self.video_capture_right.read()
        ret_left, self.frame_left = self.video_capture_left.read()
        if ret_right and ret_left:
            frame = cv2.vconcat([self.frame_left, self.frame_right])
            self.image = frame.copy()
            self.YOLOv7_detect()

            self.corrected_msg = self.bridge.cv2_to_imgmsg(frame, "bgr8")
            self.corrected_publisher.publish(self.corrected_msg)

        # print("hz = ", 1.0/(time.time()-self.time_))
