...
class ObjectDetection(Node):
    def __init__(self, img_size: Union[Tuple[int, int], None] = (640, 640)
        ...
        # Parameters
        self.declare_parameter("weights", "/home/ccm/ros2_ws/src/object_detection/object_detection/cone_best.pt", ParameterDescriptor(description="Weights file"))
        self.declare_parameter("classes_path", "/home/ccm/ros2_ws/src/object_detection/object_detection/labels_last_coord.txt", ParameterDescriptor(description="Class labels file"))
        ...
        # Publishers for count
        self.detect_len_publisher = self.create_publisher(Int16, "/yolo_count", 10)
        self.detect_len_msg = Int16()

        # Publishers for detection
        self.detect_publisher = self.create_publisher(Detection2DArray, "/yolo_detect", 10)
    ...
    def YOLOv7_detect(self):
        ...
        if len(detections) == 0:
            ...
            # self.detect_len_msg = len(detections)
            # self.detect_len_publisher.publish(self.detect_len_msg)
            #print(type(self.detect_len_msg))

            # detect_msg = detections.flatten()
            # self.detect_publisher.publish(detect_msg)
            #classes = [int(c) for c in detections[:, 5].tolist()]
            #conf = [float(c) for c in detections[:, 4].tolist()]
            # list to int
            #print(type(classes),",",type(classes[0]),",",type(int(classes[0]))) # list, int, int
            #print(self.class_labels[classes[0]])
            #print(self.classes[classes[0]])
            #print(conf, ",", type(conf[0]))

            # 욜로 탐지 개수 o
            self.detect_len_msg.data = len(detections) # 정말 중요!!!! 선언한 메세지 뒤에 .data붙이기
            self.detect_len_publisher.publish(self.detect_len_msg) 
            ...
            print(box_info, "\n",len(box_info))
            print(type(box_info[0][5])) # class float, 0:blue, 1:orange
        ...
        if self.visualization_publisher:
            ...
            self.vis_msg = self.bridge.cv2_to_imgmsg(vis_img)
    ...
    def img_callback(self, data):
        ...
        video = self.image.copy()
        cv2.imshow('frame', video)
        if cv2.waitKey(1) & 0xFF == ord('q'):
            cv2.destroyAllWindows()
        self.sub_img_flag = True
        #self.YOLOv7_detect(data)
